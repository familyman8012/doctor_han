...
### 15-1. 계정/승인 플로우 마감
- [ ] 초대 링크 기반 회원가입 도입 여부 확정
  - [ ] 옵션 A: 초대 링크 기반 회원가입 구현
  - [ ] 옵션 B: 클로즈드 베타 기간에는 운영 수동 등록으로 대체하고 정책 문서화
- [ ] 관리자 승인 후 활성화 플로우 완결
  - [ ] `doctor/vendor` 승인 시 실제 사용 가능 상태까지 전이되도록 정리
  - [ ] `profiles.status`, `vendors.status`, 검수 상태 간 책임 분리
  - [ ] 비활성/정지 계정의 핵심 API 접근 차단 규칙 보강
