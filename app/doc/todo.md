...
- [ ] 리드 알림 정책 확정
  - [ ] “문자”를 실제 SMS로 할지, 이메일+카카오 알림톡으로 운영할지 결정
  - [ ] 선택한 채널 기준으로 발송 실패 시 재시도/대기열/운영 알림 정책 정의
- [ ] 리드 생성 후 업체 수신 보장 강화
  - [ ] 현재 best-effort 알림 구조를 운영 추적 가능한 구조로 보강
  - [ ] 알림 실패가 발생했을 때 관리자에서 즉시 확인 가능하도록 처리
- [ ] 리드 열람 시각(`viewed_at`) 기록 추가
- [ ] 미열람 리드 수 집계 추가
- [ ] `24시간 이상 미열람`, `72시간 미열람/미응답` 정책을 코드 기준으로 재정의
- [ ] 상태 변경 시 의료인 알림 필요 여부 확정 및 구현
