...
> 접근: 페이지별 단계적 개선 — 메인 → 카테고리 리스트 → 업체 상세 → 공통 디자인 시스템

### 14-1. 디자인 시스템 기반 정비
- [ ] 컬러 팔레트 정리: Primary(#0a3b41/#62e3d5) 외 Secondary/Accent/Semantic 색상 체계 확립
- [ ] 타이포그래피 스케일: heading(h1~h4)/body/caption/label 크기·weight 규칙 문서화 + Tailwind preset
- [ ] 간격(spacing) 시스템: 4px 기반 그리드, section 간격/카드 내부 패딩 규칙 통일
- [ ] 그림자(elevation) 단계: card/dropdown/modal/toast 4단계 shadow 토큰
- [ ] 아이콘 체계: lucide-react 기본 + 카테고리별 커스텀 아이콘 (현재 이모지/SVG 혼용 → 통일)
- [ ] 버튼 변형: Primary/Secondary/Ghost/Danger + size(sm/md/lg) 매트릭스 정리
- [ ] 반응형 breakpoint 규칙: mobile(375)/tablet(768)/desktop(1280)/wide(1440) 기준 정리
- [ ] 다크모드 대응 여부 결정 (P9에서는 라이트 모드만 완성, 다크는 선택)
...
- [ ] 필터 항목 추가:
  ...
  - [ ] 배지 필터 (인증업체 / 프리미엄 등)
- [ ] 정렬 옵션 추가: 최신순 / 평점순 / 리뷰많은순 / 응답빠른순 / 인기순
- [ ] 필터 UI 개선: 크몽식 사이드바 필터 또는 상단 드롭다운 조합
- [ ] 선택된 필터 태그 표시 + 초기화 버튼
- [ ] 필터 결과 카운트 실시간 업데이트 ("N개 업체")
- [ ] (Backend) 필터/정렬 API 확장: `GET /api/vendors` 쿼리 파라미터 추가
...
- [ ] (Backend) 탭별 데이터 lazy loading 또는 한 번에 로드
...
- [ ] 현재 plain text → 리치 콘텐츠 지원:
  ...
  - [ ] 서비스 특장점 아이콘+텍스트 리스트
  - [ ] FAQ 아코디언 (자주 묻는 질문)
- [ ] (Backend) 업체 소개 필드를 리치텍스트(markdown/html)로 확장
  - [ ] `PATCH /api/vendors/me` description 필드 확장 또는 별도 `rich_description` 추가
...
- [ ] 사례(케이스 스터디) 형태 지원:
  ...
  - [ ] 시공 전/후 비교 이미지
  - [ ] 프로젝트 설명 (규모/기간/비용)
- [ ] (Backend) 포트폴리오 메타데이터 확장: title, description, tags, is_featured
...
- [ ] (Backend) `vendor_service_prices` 데이터를 상세 페이지에서 노출
...
- [ ] (Backend) 리뷰 항목별 세부 평점 필드 추가
  - [ ] DB: `reviews` 테이블에 `quality_rating`, `communication_rating`, `speed_rating` 추가
  - [ ] API: `POST /api/reviews` 확장, `GET /api/vendors/:id/reviews` 응답 확장
- [ ] (Backend) 업체 답변 기능
  - [ ] DB: `review_replies` 테이블 (또는 `reviews.vendor_reply` 필드)
  - [ ] API: `POST /api/reviews/:id/reply` (업체 전용)
...
### 14-6. 신뢰/배지 시스템 (Backend + Frontend)
- [ ] 배지 종류 정의:
  - [ ] "메디허브 인증" — 사업자 인증 완료
  - [ ] "빠른 응답" — 평균 응답시간 24h 이내
  - [ ] "Top N%" — 카테고리 내 리뷰/평점 상위
  - [ ] "프리미엄 파트너" — 유료 멤버십 업체
  - [ ] "신규 입점" — 입점 30일 이내
- [ ] (Backend) 배지 자동 계산 로직:
  - [ ] DB: `vendor_badges` 테이블 또는 `vendors.badges` jsonb 컬럼
  - [ ] Cron/트리거: 응답시간/리뷰/멤버십 기반 배지 자동 부여/회수
  - [ ] API: `GET /api/vendors/:id` 응답에 배지 목록 포함
- [ ] (Frontend) 배지 UI 컴포넌트:
...
- [ ] 검색 결과 없음 → 유사 업체 추천
- [ ] (Backend) 검색 인덱스 최적화 (Supabase full-text search 또는 별도 인덱싱)
...
### 14-10. 이미지 에셋 제작 (AI 생성 or 스톡)
- [ ] 카테고리 아이콘 세트 (9개 업종)
- [ ] 히어로 배너 이미지 (3~5장)
- [ ] 카테고리 리스트 상단 배경 이미지 (9개)
- [ ] 업체 기본 프로필 이미지 (카테고리별 9종)
- [ ] 빈 상태(empty state) 일러스트 (검색 결과 없음, 리뷰 없음, 포트폴리오 없음 등)
- [ ] 온보딩/가이드 일러스트
- [ ] (도구) Replicate MCP 연동으로 AI 이미지 생성 파이프라인 구축
