...
### 3-1. 공통
- [ ] 공통 응답/에러 포맷 확정(프론트 에러 중앙화와 호환)
- [ ] 인증/권한 가드(roles) 헬퍼 구현
- [ ] 입력 검증(Zod) 패턴 확정(`src/lib/schema` 신설)
...
### 4-1. 공통 레이아웃/내비게이션
- [ ] 헤더/검색창/카테고리 진입 동선
- [ ] 역할별 가드(guest/doctor/vendor/admin)

### 4-2. 회원가입/로그인(MVP)
- [ ] 일반 회원가입(이메일/비번) + 한의사 프로필 입력 + 면허증 업로드(검수 대기)
- [ ] 사업자 회원가입(이메일/비번) + 업체 기본정보/담당자 + 사업자등록증 업로드(검수 대기)
- [ ] 로그인/로그아웃
- [ ] 계정찾기(후순위)
- [ ] 휴대폰 본인인증/소셜 로그인(후순위)
...
- [ ] 카테고리 트리 UI
- [ ] 키워드 검색(업체명/소개/태그 등)
- [ ] 업체 리스트(카드) + 필터(가격/평점/배지) + 정렬(최신/응답순 등은 데이터 준비 후)
- [ ] 업체 상세(포트폴리오/가격/FAQ/리뷰)
- [ ] 찜/공유(공유는 후순위 가능)

### 4-4. 리드/문의(MVP)
- [ ] 문의 생성 폼(필수 입력 + 첨부)
- [ ] 한의사 “내 문의함”(상태/메모/첨부/취소)
- [ ] 업체 “받은 리드함”(상태 관리/견적/응답)
- [ ] 상태 자동화/SLA 알림(후순위)

### 4-5. 마이페이지/파트너센터(MVP)
- [ ] 한의사: 프로필 수정/찜 리스트/리뷰 작성/조회(수정/삭제/사진/비공개 전환은 P1+)
- [ ] 업체: 업체 프로필 관리/포트폴리오 관리/리드 대응
...
- [ ] 휴대폰 본인인증(PASS) 연동 + 결과 저장
- [ ] 소셜 로그인: 네이버/카카오/구글(연동 후 자동가입/로그인)
...
- [ ] 리뷰 고도화: 정렬/수정/삭제/사진/비공개 전환, 노출 정책 안내
