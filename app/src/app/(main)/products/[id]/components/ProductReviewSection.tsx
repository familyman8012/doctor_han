...
import { ReviewReportModal } from "../../../vendors/[id]/components/modal/ReviewReportModal";
...
export function ProductReviewSection({ productId, ratingAvg, reviewCount, currentUserId }: ProductReviewSectionProps) {
    ...
    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- productId 변경 시 페이지 초기화 필요
        setPage(1);
    }, [productId]);
    ...
    const { data: reviewData, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            const response = await api.get<{
                data: { items: (ReviewView & { reply: unknown })[]; page: number; pageSize: number; total: number };
            }>(`/api/products/${productId}/reviews?page=${page}&pageSize=${PAGE_SIZE}&sort=${sort}`);
